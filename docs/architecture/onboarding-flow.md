...
> Updated 2026-06-29 — BLI-306: Step 1 gains a required binary gender selector (Kobieta/Mężczyzna, testIDs `gender-female`/`gender-male`) beside the 18+ toggle; "Dalej" now also gates on it. Stored in `onboardingStore.gender` and sent through `applyProfile`. Nullable `profiles.gender` column, store-only (no display/filter/matching). All onboarding e2e flows tap a gender pill.
