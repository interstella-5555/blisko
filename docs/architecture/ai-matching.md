...
> Updated 2026-04-11 — Modal profilu (mobile) zhookowany na `getDetailedAnalysis` (wcześniej wołał `getConnectionAnalysis` — read-only, bez promocji T3). `ensureAnalysis` i `getDetailedAnalysis` sprawdzają `tier === 't3'` jako readiness, nie "row exists". Tier invariant udokumentowany explicite (BLI-188).
