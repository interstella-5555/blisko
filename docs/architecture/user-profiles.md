...
> Updated 2026-06-29 — BLI-306: `profiles.gender` added (nullable `"female"`/`"male"`), collected in onboarding Step 1. Optional in `createProfileSchema`/`applyProfilingSchema`/`createGhostProfileSchema` (required only in the mobile UI); written conditionally so settings re-profiling never nulls it. Store-only — not displayed, filtered, or matched on yet. See `database.md` + `onboarding-flow.md`.
...
| Status "na teraz" | `currentStatus` + `statusCategories` | Status |
...
- Status is always public: shows status if `isStatusActive(profile)` --- i.e. `currentStatus !== null` --- for own and other profiles alike
- `getMyStatusMatches`: always returns the actual LLM `reason` (no redaction)

**`clearStatus` side effects:** Nullifies all status fields (`currentStatus`, `statusExpiresAt`, `statusEmbedding`, `statusCategories`, `statusSetAt`) AND deletes all status matches involving the user from `statusMatches` table.
