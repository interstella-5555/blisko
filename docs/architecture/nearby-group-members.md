...
- **`apps/api/src/config/rateLimits.ts`** — `profiles.getNearby` rate limit (30/min) applies to map refresh which triggers `getDiscoverable`. Consider if `getNearbyMembers` needs its own limit for large groups.
