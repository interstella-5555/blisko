...
**Required GitHub secrets:** `EXPO_PUBLIC_API_URL`, `ANDROID_KEYSTORE_BASE64`, `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS`, `ANDROID_KEY_PASSWORD`, `PLAY_STORE_SERVICE_ACCOUNT_JSON`. Keystore + service-account files are written to disk at job start and deleted in the `always()` cleanup step.
