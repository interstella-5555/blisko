...
All 8 stores in `apps/mobile/src/stores/`. Every store must be categorized in `src/stores/reset.ts` as either **user-scoped** (wiped at session boundary via `resetUserScopedStores()`) or **device-scoped** (preserved across sessions). Enforcement is by convention and code review (rule `mobile/new-store-categorize` in `.claude/rules/mobile.md`); default for new stores is user-scoped — opt out only when the data is device state (GPS) or a UX preference the product wants to survive logout (notification prefs, nearby radius).
