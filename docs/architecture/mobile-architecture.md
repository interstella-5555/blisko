...
> Updated 2026-06-20 — BLI-291 nearby UX polish: light `hapticTap()` helper (`src/lib/haptics.ts`, `Haptics.impactAsync(Light)`) wired on `onPressIn` for the count pill, recenter, status pill and filter button, plus a `tabPress` listener on the bottom tabs. Recenter icon swapped from `IconPin` to a new `IconNavigate` (Lucide "navigation" ↗). Note: the iOS Simulator has no Taptic Engine, so haptics are device-only.
...
> Updated 2026-04-21 — BLI-255 Android map avatars fix. Native patch of `react-native-maps` 1.27.2 (`patches/react-native-maps@1.27.2.patch`, persisted via `bun patch` + root `package.json` `patchedDependencies`). Upstream bug: under Fabric, `MapMarker.addView` only hooks the Fresco `DraweeView` lifecycle at the first nesting level — child `<Image>` rendered deeper in the tree (e.g. inside `<View>`/`PulsingWrapper` as in `GridClusterMarker`) resolves to a Glide-backed `AppCompatImageView` that never triggers a marker re-snapshot, so the `BitmapDescriptor` freezes on the pre-load empty container → blank white circles. Patch adds `hackToHandleImageLifecycleRecursive` that walks the child tree and force-loads Glide images via reflection, calling `update(true)` on `onResourceReady`. iOS unaffected (`MKAnnotationView` renders a live `UIView`). Upstream issue: [react-native-maps#5784](https://github.com/react-native-maps/react-native-maps/issues/5784). See "Gotchas" below.
...
> Updated 2026-04-21 — BLI-252: `(tabs)` layout moved from React Navigation's native header to a custom `TabHeader` primitive (`src/components/ui/TabHeader.tsx`) — fixes Android title left-alignment and brings tabs in line with the `mobile/no-native-headers` rule already followed by modals / settings / chat / create-group / onboarding. Also inlined `tabBarLabelStyle` → `type.tabLabel` token.
...
> Updated 2026-05-20 — BLI-281: post-login screens migrated to Lingui (tabs, chat, settings, modals, status / filters / create-group / mute / group-join, in-app notifications, UI primitives). Backend push notifications now locale-aware via `apps/api/src/services/i18n.ts` (`t(key, locale, params)` + flat JSON catalogs at `apps/api/src/locales/{pl,uk}.json`) — every push callsite fetches the recipient's `profiles.locale` and passes it through. See `docs/architecture/push-notifications.md` for the per-trigger table.
...
> Updated 2026-06-28 — BLI-305 map controls + settings tab. Removed the top floating row — the status pill is gone, so the active status is no longer shown on the map (it lives only in the set-status sheet). Filters moved out of the top bar into a bottom-right vertical action column (`fabColumn`), ordered recenter → filters → primary `+`. The `+` (accent FAB) is the primary action: it opens the set-status sheet (prefilled when a status is already set) and carries the `set-status-pill` testID inherited from the removed top pill. Recenter now fades in (opacity + scale `Animated`) only when the map has drifted >15% of the viewport off the user's location (`showRecenter`) — i.e. only when tapping it would move the map. The count pill is left-aligned and reserves room for the column. Removed the "Pingi: X/Y" counter and its `waves.getQuota` query. The bottom "Profil" tab became an "Ustawienia" launcher (gear icon) that `router.push("/settings")` via a `tabPress` `preventDefault` listener; `(tabs)/profile.tsx` is now a `<Redirect href="/settings" />` — the standalone own-profile view was removed. E2E updated: map-render asserts `nearby-count-pill` (recenter is conditional now); set-status verifies persistence by reopening the prefilled sheet (the on-map `active-status-pill` no longer exists). PL/UA parity kept.
...
|   +-- profile.tsx                 "Ustawienia" tab --- launcher that opens /settings (redirect; own-profile view removed, BLI-305)
|
...
**Why no DB → store sync.** Pulling DB on login would clobber a user who deliberately picked a different language on this phone — e.g. someone with a PL phone for work and a UA phone for personal use. The DB value reflects "whichever device most recently logged in"; that's the right semantics for server-side rendering (the user wants their next email in the language they're currently using) but the wrong semantics for cross-device UI propagation.
...
**Custom headers (no native headers):** iOS wraps `headerLeft`/`headerRight` in `UIBarButtonItem` with an ugly capsule background that cannot be removed; React Navigation's native header also defaults `headerTitleAlign` to `left` on Android vs `center` on iOS, so titles drift cross-platform if the native header is used. All headers use `header: () => (...)` with `SafeAreaView` + centered title + back chevron pattern. Height normalized via `layout.headerHeight = 44` (iOS nav bar standard) from `theme.ts`. Three flavors in the codebase: (1) `TabHeader` primitive — shared by `(tabs)`, `settings`, and `(modals)` layouts; (2) bespoke inline headers for screens with non-slot-shaped content — chat screen (avatar + multi-line name + mute toggle), create-group inline header; (3) `OnboardingStepHeader` — different visual language (chevron paired with label text, optional logout pill, no bottom divider).

**TabHeader primitive:** `src/components/ui/TabHeader.tsx` — SafeAreaView + bottom-rule divider + 3-slot row (24px / centered title / 24px rightAction). Props: `title: string`, optional `leftAction?: ActionConfig | "back"`, optional `rightAction?: ActionConfig`. `ActionConfig` = `{ Icon: ComponentType, onPress, testID? }` — `Icon` is a component (not an element) so the header owns icon size and color per slot (left: 24 / `colors.ink`, right: 20 / `colors.muted`), preventing per-call-site drift. The `"back"` sugar on `leftAction` resolves internally to `{ Icon: IconChevronLeft, onPress: () => router.back() }` and encodes the `mobile/back-button` rule (chevron + size 24 + `colors.ink` + `hitSlop={8}`). Used by bottom tabs (`(tabs)/_layout.tsx`) and by both Stack layouts that need a back chevron: `settings/_layout.tsx` and `(modals)/_layout.tsx`. Pre-BLI-252 the tabs used React Navigation's native header (Android title left-aligned); pre-BLI-253 settings + modals inlined the same JSX three ways.
...
### `react-native-maps` Fabric marker child Image bug (Android only)

`NearbyMapView` renders users and groups as `<Marker>`s with a child React tree containing an `<Image>` (avatar) nested inside `<View>`s (white border, shadow, pulsing wrapper for `statusMatch`). On Android this rendered as blank white circles until BLI-255.

**Root cause.** The Google Maps SDK on Android draws markers from a pre-rendered `BitmapDescriptor`, not a live view (unlike iOS `MKAnnotationView`). `react-native-maps` snapshots the marker child view on `addView()` and caches the result. The library's `MapMarker.addView()` contains a hack — `hackToHandleDraweeLifecycle` — that re-snapshots once the Fresco `DraweeView` loads its bitmap. Under Fabric the hack only checks the direct child, but our `<Image>` is wrapped two levels deep, and the Fabric codegen routes `<Image>` through a Glide-backed `AppCompatImageView` (not Fresco `DraweeView`) anyway. Neither condition matches → no re-snapshot → marker stays on the pre-load empty container.

Cluster badges (text-only) and initial-letter placeholders (`<View>` + `<Text>`) render fine — the bug is specific to nested async image loads.

**Fix.** `patches/react-native-maps@1.27.2.patch` replaces `hackToHandleDraweeLifecycle(child)` in `addView()` with `hackToHandleImageLifecycleRecursive(child)`. The new method walks the view tree and, for each non-Drawee `ImageView`, uses reflection to pull the `mSource` ReadableMap, kicks off a Glide load, and triggers `clearDrawableCache() + update(true)` on `onResourceReady`. The patch is persisted via `bun patch` (root `package.json` `patchedDependencies`) and re-applies on every `bun install`.
