...
> Updated 2026-04-20 — BLI-243 follow-ups: (a) extracted `AppGate` + nested `LocationGate` into `src/components/AppGate.tsx` — auth / profile-fetch / location-fix resolution lives in a SINGLE `<SplashHold>` instance that wraps the root `<Stack>`, so the SonarDot ring animation never restarts between cold-launch phases. Replaces the previous per-group-layout splash gates in `(tabs)`, `(auth)`, `onboarding` (all three layouts now rely on the root gate). (b) `locationStore` moved from `DEVICE_SCOPED_STORES` → `USER_SCOPED_STORES` — the cached GPS fix is user-generated and wiped on logout. Now persists via SecureStore (same adapter as `onboardingStore`) so returning users see the map at their last known position immediately. `permissionStatus` is persisted too so `LocationGate` can decide whether to hold the splash on cold launch. `(tabs)/index.tsx` loader condition simplified to `!latitude || !longitude` (the `undetermined` branch no longer triggers on returning users with a cached fix). (c) `authStore.Profile` type extended with `portrait` + `updatedAt` so the AI-pipeline stale-check in `(tabs)/_layout.tsx` can read from the store instead of a second copy of the `profiles.me` query (which now lives solely in `AppGate`).
...
User's current GPS position. Persisted to SecureStore (`blisko-location`, same adapter as `onboardingStore`) so returning users see the map at their last known fix on cold launch while a fresh fix comes in from `Location.watchPositionAsync` in the background. Schema: `latitude` / `longitude` / `lastUpdate` (epoch ms) / `permissionStatus` (`undetermined` / `granted` / `denied`). User-scoped — wiped on logout so a previous account's position doesn't leak to the next sign-in on the same device. Consumed at the root by `LocationGate` (inside `<AppGate>`) which holds the splash when the user is authenticated + permission is `granted` + no cached fix exists yet (narrow window after first-grant, before first fix).
...
**Gate chain.** After fonts load, `app/_layout.tsx` renders providers → `<AppGate>` → `<Stack>`. `AppGate` (source: `src/components/AppGate.tsx`) is a **single** `<SplashHold>` instance that holds the splash up across three sequential phases so the `SonarDot` ring animation never restarts:
...
3. **Location fix** — nested `LocationGate` inside `AppGate` holds when `user` + `hasCheckedProfile` + `permissionStatus === "granted"` + no cached lat/lng. `denied` / `undetermined` fall through so `(tabs)/index` can render the system prompt or the "Brak dostępu" screen in context.
