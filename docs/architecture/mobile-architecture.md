...
**Tap → chat routing.** All three chat-opening toast onPress handlers (`newMessage`, `waveResponded` accepted, `groupInvited`) route through a shared `openChatFromNotification(conversationId, currentPathname)` helper that (1) `router.dismissAll()` guarded by `canDismiss()` — clears any open modal or form-sheet; unguarded on tabs root it dispatches a `POP_TO_TOP` no navigator can handle and logs an "unhandled action" warning; (2) `router.navigate("/(tabs)/chats")` skipped when `currentPathname === "/chats"` — a redundant navigate to the current tab jumps focus to the last tab (Profil) for reasons not fully traced; behaviourally observed on simulator, hence the skip; (3) `router.push('/chat/${conversationId}')`. Net effect: regardless of where the user was when the toast fired (map, profile modal, privacy settings, another chat, form sheet), swiping back from the opened chat lands on the Czaty tab — chat opens feel like they come from the chat list. `pathname` is read via `usePathname()` and threaded into the memoised handler dep list so the helper sees the current route at tap time.
