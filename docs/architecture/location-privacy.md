...
> Updated 2026-05-24 — BLI-287: `getNearbyUsersForMap` now exposes `lastLocationUpdate` on each user's `profile` object, and `getById` already returned it inline (it's a column on `schema.profiles`). Mobile renders this through `formatLastActive` in `apps/mobile/src/lib/format.ts` as "teraz" / "X min temu" / "X godz. temu" / "wczoraj" / "X dni temu" / "dawno temu" — both on the nearby list (`UserRow`) and the profile modal header. **Semantics:** "user was foreground-active at this timestamp", because `lastLocationUpdate` is written by the `profiles.updateLocation` mutation which fires on nearby-tab mount + 3 s retry only (foreground-only). It is NOT a real-time presence signal. The "teraz" 5-minute window approximates "the other user has the app open right now", which is close enough until the v4 GPS-on-stationary model lands. `getNearbyMapMarkers` deliberately does not include `lastLocationUpdate` — bubbles are neutral per v4 §6.2, recency surfaces in the list/profile views only.
