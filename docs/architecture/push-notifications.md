...
> Updated 2026-05-20 — BLI-281: push notification bodies are now locale-aware. `apps/api/src/services/i18n.ts` exposes `t(locale, { pl, uk })` — both translations are inlined at the callsite (`waves.ts`, `groups.ts`, `messages.ts` group-aggregate-unread, `queue.ts` ambient match). Each handler fetches the recipient's `profiles.locale` and passes it in; nulls fall back to PL. DM single-message and group first-unread bodies stay locale-agnostic (user content + `:` separator).
...
Bodies pass through `t(locale, { pl, uk })` from `apps/api/src/services/i18n.ts` (see [`i18n.md`](./i18n.md)) using the recipient's `profiles.locale`. Recipients with `profile.locale === null` (or any unsupported locale) fall back to PL. Both PL and UA strings are inline at the callsite — no central catalog file. Templates with no localizable content (DM single-message body, group first-unread body) are passed through directly without `t()` — they're just user content + a `:` separator.

| Trigger | Title | Body (PL → UA, interpolated inline) | collapseId | Sound | Source file |
|---|---|---|---|---|---|
| New wave | `"Blisko"` | `"{senderName} — nowy ping!"` / `"{senderName} — новий пінг!"` | none | Yes | `waves.ts` |
| Wave accepted | `"Blisko"` | `"{responderName} — ping przyjęty! Możecie teraz pisać."` / `"{responderName} — пінг прийнято! Тепер можете писати."` | none | Yes | `waves.ts` |
...
| Group message (has unreads) | `"{groupName}"` | `"{unreadCount} nowych wiadomości"` / `"{unreadCount} нових повідомлень"` | `"group:{conversationId}"` | No (silent) | `messages.ts` |
| Ambient status match | `"Blisko"` | `"Ktoś z pasującym profilem jest w pobliżu"` / `"Хтось із відповідним профілем поблизу"` | `"ambient-match"` | No (silent) | `queue.ts` |
| Group invite (create) | `"{groupName}"` | `"Nowe zaproszenie do grupy"` / `"Нове запрошення до групи"` | `"group-invite:{conversationId}"` | No (silent) | `groups.ts` |
| Group invite (addMember) | `"{groupName}"` | `"Nowe zaproszenie do grupy"` / `"Нове запрошення до групи"` | `"group-invite:{conversationId}"` | No (silent) | `groups.ts` |
