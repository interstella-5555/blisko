...
| `trpcServer` `onError` callback, filtered to `INTERNAL_SERVER_ERROR`, capturing `error.cause ?? error`. Same handler also writes a structured `[trpc:error] path=… code=… cause=… message=… <stack>` line to stdout so Railway logs (`@level:error` filter) surface failures even if Bugsink ingest is down. Necessary because batched tRPC requests return HTTP 200 with errors in the body — the Hono access log can't see them. The `user.id` on captured events comes from two sources: `tagSentryUser` middleware on every `protectedProcedure` (`apps/api/src/trpc/trpc.ts`) calls `Sentry.setUser` on the request scope so any capture during the lifecycle picks it up, and the `onError` callback also passes `user` per-call as a backstop in case the AsyncLocalStorage context doesn't reach the adapter callback. | `src/index.ts`, `src/trpc/trpc.ts` | `source: trpc`, `path`, `type`; `user.id` |
