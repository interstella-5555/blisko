...
> Updated 2026-04-19 — Audit follow-up. Chatbot now sends `serviceTier: "standard"` + `reasoningEffort: "minimal"` in `logAiCall` payloads (was implicitly defaulting to `standard` / `null` even though the actual OpenAI call uses minimal reasoning) — `byJobNameAndTier` and `byServiceTier` breakdowns now correctly attribute chatbot calls. Chatbot `input.maxOutputTokens` log entry also synced to actual call value (was 150, real call uses 500).
