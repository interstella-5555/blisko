...
**Goal:** Add privacy policy and terms of service pages to the website, link them from the mobile app's registration and settings screens.

**Architecture:** Static HTML pages served from the Bun website server (`apps/website/src/index.ts`). Mobile app gets acceptance text on login screen and updated links in help screen.

**Tech Stack:** Bun (website), React Native (mobile), Expo Router

---

### Task 1: Add `/privacy` and `/terms` routes to website

**Files:**
- Modify: `apps/website/src/index.ts`

**Step 1: Add `privacyPage()` function**

Add after the `homePage()` function. Returns a full HTML document with the privacy policy content in Polish. Follow the same style as `joinPage()` — inline CSS, `<html lang="pl">`, responsive layout.
...
**Step 2: Add `termsPage()` function**

Add after `privacyPage()`:
...
**Step 3: Add routes to the fetch handler**

In the `fetch(req)` handler, add before the `// Home` comment (around line 168):
...
Run: `cd apps/website && bun run src/index.ts`
Open: `http://localhost:3001/privacy` and `http://localhost:3001/terms`
Expected: Both pages render with Polish content, responsive layout, back link to blisko.app

**Step 5: Commit**

```bash
git add apps/website/src/index.ts
git commit -m "Add privacy policy and terms pages to website (BLI-67)"
```

---

### Task 2: Update help screen links

**Files:**
- Modify: `apps/mobile/app/settings/help.tsx:44-56`

**Step 1: Update URLs**

Change lines 48 and 55:

```tsx
// Line 48: change from
Linking.openURL("https://blisko.app/regulamin");
// to
Linking.openURL("https://blisko.app/terms");

// Line 55: change from
Linking.openURL("https://blisko.app/prywatnosc");
// to
Linking.openURL("https://blisko.app/privacy");
```

Also remove the TODO comments on lines 47 and 54.

**Step 2: Commit**

```bash
git add apps/mobile/app/settings/help.tsx
git commit -m "Update privacy and terms links in help screen (BLI-67)"
```

---

### Task 3: Add acceptance text on login screen
...
Add `Linking` to the React Native import on line 4:

```tsx
import { Linking, Platform, Pressable, StyleSheet, Text, View } from "react-native";
```

**Step 2: Add acceptance text after the form View**

After the closing `</View>` of `styles.form` (line 220) and before the closing `</View>` of `styles.content` (line 221), add:
...
Run the mobile app. On the login screen, the acceptance text should appear below the email button, small and muted, with tappable underlined links.

**Step 5: Commit**

```bash
git add apps/mobile/app/\(auth\)/login.tsx
git commit -m "Add terms and privacy acceptance text on login screen (BLI-67)"
```
