...
The GitHub Actions workflow `.github/workflows/android-deploy.yml` runs `expo prebuild --platform android`, copies this folder into `apps/mobile/android/fastlane/`, then runs `fastlane deploy_internal`. Fastlane is installed via `gem install` (no Bundler — avoids a double-init conflict with Ubuntu's system Ruby).
