...
import { type WaveTab, WaveTabBar } from "../../src/components/waves/WaveTabBar";
...
export default function WavesScreen() {
  ...
  const pendingReceivedCount = receivedWaves.filter((w) => w.wave.status === "pending").length;
  const pendingSentCount = sentWaves.filter((w) => w.wave.status === "pending").length;

  const filteredSent = useMemo(() => sentWaves.filter((w) => w.wave.status !== "declined"), [sentWaves]);
  ...
  const renderReceivedList = () => (
    <FlatList
      ...
      ListEmptyComponent={hydrated && !refreshing ? <EmptyWavesState type="received" /> : null}
  ...
  const renderSentList = () => (
    <FlatList
      ...
      contentContainerStyle={styles.list}
      ListEmptyComponent={hydrated && !refreshing ? <EmptyWavesState type="sent" /> : null}
      refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.ink} />}
    />
  );
